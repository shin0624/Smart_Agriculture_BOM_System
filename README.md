...
[부품 등록 버튼]
      ...
      ▼
[부품 등록 다이얼로그]
① 기본 정보: 부품명, 부품번호, 카테고리 선택, 제조사 선택
② 이미지: 파일 선택 → 자동 리사이즈 후 assets/parts/ 저장
③ 규격 정보: [+ 추가] 버튼으로 Key-Value 행 동적 추가
   예) [크기] [150x80mm] [삭제]
       [재질] [주철    ] [삭제]
④ 적용 농기계: 체크박스 다중 선택 + 모델 메모
⑤ 연관 부품: 부품 검색 후 관계 유형 선택하여 추가
⑥ 재고/단가: 초기 재고수량, 단가
      |
      ▼
[저장] → DB INSERT
...
```text
┌─────────────────────────────────────────────────────────────────────────┐
│  스마트 농업기계 BOM 시스템                         [최소화][닫기]      │
├─────────────────────────────────────────────────────────────────────────┤
│  [부품 검색]  [재고 관리]  [판매 기록]  [기준 정보]                     │
├──────────────┬──────────────────────────────────────────────────────────┤
│ [검색 패널]  │              [상세/결과 패널]                             │
│              │                                                           │
│ 검색어: [  ] │                                                           │
...
│ [+ 신규등록] │                                                           │
└──────────────┴──────────────────────────────────────────────────────────┘
│ 총 부품: 248 종  |  재고 부족 경고: 5 건  |  오늘 판매: 3 건            │
└─────────────────────────────────────────────────────────────────────────┘
```
